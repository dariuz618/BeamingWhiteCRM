...
");
    ...
    /* Dariuz Rubin */
    /**
	* Get the names of products that were saved in account 
	*/
     public function user_productname($userId) {
        return $this->_db->fetchAll("SELECT distinct product.name as productName,product.product_id FROM product_price left join product on product_price.product_id = product.product_id where product_price.user_id = '$userId' AND product_price.active = 1 ORDER BY productName;");
    }
    
    
    /* Get User Products */
     public function getUserProducts() {
     	$products = array('Teeth Whitening Packages','Futura Face','Futura 2400','Private Labeling','Home Whitening Products','In Office Whitening','Syringes','Kit Components','Tan White','Evoke Masks','Collagen Bed','Skin-FX','Sapient Dental Products','Sonic-FX Tooth Brush');
        return $products;
    }    
    /*==============*/
}
