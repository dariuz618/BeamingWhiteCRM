...
class FmController extends Zend_Controller_Action {
    ...
    public function downloadAction() {        
	 ...
	 $this->_helper->viewRenderer->setNoRender(true);
         ...
         if ($this->_getParam('type') == 'salesfile') {
            $file = $this->_fm->getSalesFileById($this->_getParam('id')); 
            $path = 'data/downloads/' . $file['url'];
    ...
    public function salesFileUploadAction() {
        
    }
    public function salesFileCategorizeAction() {       
...
//die('{"jsonrpc" : "2.0", "result" : {"cleanFileName": "'.$fileName.'"}, "id" : "id"}');
    ...
    public function deleteAction() {
        ...
        if (isset($_POST['id'])) {            
            ...
            if (isset($_POST['type']) && $_POST['type'] == 'salesfile') {
                $thisFile =  $this->_fm->getSalesFileById($_POST['id']);
                $filePath = 'data/downloads/' . $thisFile['url'];
                $fileThumb = 'data/downloads/' .'thumb_'. $thisFile['url'];
            } else {
            ...
            if ($thisFile && file_exists($filePath)) {
                if (unlink($filePath)) {                    
                     if (isset($_POST['type']) && $_POST['type'] == 'salesfile') {
                         ...
                         if ($this->_fm->deleteSalesFile($_POST['id']))
                            echo 'success';                         
                     } else {
    ...
    public function salesFileListAction() {
       // $this->view->targetDir = $this->_downloadsDir . DIRECTORY_SEPARATOR;
        $this->view->categories = $this->_fm->getSalesFilesCategory();  
        $this->view->category = $this->_getParam('category');
        $this->view->files = $this->_fm->getSalesFiles($this->view->category); 
        $this->view->fileDir = 'data/downloads/';
    ...
    public function salesFilePublicAction() {          
         $this->_helper->layout()->disableLayout();  
         if (!in_array('download_files', unserialize($this->_auth->permission))) {
            echo 'You do not have permission to download this file!';
            die();
         ...
         $this->view->id = $this->_getParam('id');
         $file = $this->_fm->getSalesFileById($this->_getParam('id'));
         $this->view->token = $file['public_token'];         
    }
    public function salesFileDeleteAction() {   
         $ns = new Zend_Session_Namespace('Default');
         $key = array_search("{$_POST['fileId']}", $ns->item);                
         unset($ns->item[$key]);    
         $this->_helper->layout()->disableLayout();
    ...
    public function salesFileUrlAction() {        
         $this->_helper->layout()->disableLayout();
         $this->_helper->viewRenderer->setNoRender(TRUE);  
         $ns = new Zend_Session_Namespace('Default');
         ...
         if (!empty($ns->item)) {
            foreach ($ns->item as $id) {            
                ...
                echo "#$index ". $file['title'].":\n\r".             
                "http://beamingwhite.mx/downloads/index/id/".$id."/token/".$file['public_token']."\n\r";  
                ++$index;
            }     
         } else {
             echo "No file seleccted";
         }         
    }
    public function salesFileRequestsAction() {
        $this->view->requests = $this->_fm->getSalesFileRequests(); 
    }
    public function salesFileStatusAction() {
        if($_POST['action'] == 'completed') {
            $update = $this->_fm->updateSalesFileStatus(array('status' => 'completed'), $_POST['id']);
            echo 'completed';
        } elseif ($_POST['action'] == 'cancel') {
            $update = $this->_fm->updateSalesFileStatus(array('status' => 'canceled'), $_POST['id']);
            echo 'canceled';
        }             
        $this->_helper->layout()->disableLayout();
        $this->_helper->viewRenderer->setNoRender(TRUE);        
    ...
    public function salesFileRequestAction() {
        $this->view->categories = $this->_fm->getSalesFilesCategory();  
        $this->view->fileTypes = $this->_fm->getSalesFilesTypes();
        if ($this->getRequest()->isPost()) {             
          
            $data = array('category' => $this->_getParam('category'),
                          'fileType' => $this->_getParam('fileType'),                          
                          'description' => $this->_getParam('description'),
                          'priority' => trim($this->_getParam('priority')),
                          'requestedBy' => $this->_auth->firstname.' '.$this->_auth->lastname,
                          'requestedTime' => date('Y-m-d H:i:s'));              
               
            $this->view->data = $data;
            $_requriedFields = array('category' => 'Category is Required',
                                     'fileType' => 'File Type Field is Required',
                                     'description'  => 'Description Field is Required.',                                     
                                    );
            $this->error = '';
            foreach ($_requriedFields as $field => $message) {
                if (empty($data[$field])) {
            ...
            $id = $this->_fm->saveSalesFileRequest($data);
            if (is_int($id)) {
                
                $mail = new Zend_Mail();
                $message = "This is an automatic email to notify you a business file has been requested.<br>";
		                        
                $mail->setBodyHTML($message);
		$mail->setFrom('no-reply@beamingwhite.com', 'File request For Business Customers');
                $emailLists = array('luis@beamingwhite.com', 'graphics@beamingwhite.com');
                foreach ($emailLists as $email){
                  //  $mail->addTo($email);
                    $mail->addTo('jing@beamingwhite.com');
                }
                $mail->addBcc('jing@beamingwhite.com');
		$mail->setSubject('File request For Business Customers');
                $mail->send();
                
                $this->view->message = "Your request has been sent.";
                //$this->_helper->redirector('sales-file-request', 'fm');
           }
        }         
    }
    ...
    public function sendSalesFileAction() {
        ...
        if((trim($_POST['to'])) == '') {
            die("Please enter receiver's email address.");
        }
