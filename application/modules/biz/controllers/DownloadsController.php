...
class DownloadsController extends Zend_Controller_Action {
   ...
   public function init() {       
    ...
    public function indexAction() {
        ...
        $this->_fm = new Application_Model_Fm;
        $file = $this->_fm->getFileByToken($this->_getParam('token'));
        if(!$file || $file['id'] != $this->_getParam('id') ) {
            echo "This request is invalid!";
            exit();
        }
    ...
    public function salesfileAction() {
                     
        $this->_fm = new Application_Model_Fm;
        $file = $this->_fm->getSalesFileByToken($this->_getParam('token'));
        if(!$file || $file['id'] != $this->_getParam('id') ) {
            echo "This request is invalid!";
            exit();
        }                 
        $path = $this->_downloadsDir . $file['url'];  
    ...
    public function fileListsAction() {        
        $this->view->files = $this->m_user->getUserFiles($this->_auth->id);  
        $this->view->dir = $this->_downloadsDir;
    }
    //business customer files upload
      ...
      if ($this->getRequest()->isPost()) {
         ...
         $upload = new Zend_File_Transfer_Adapter_Http();
			...
			if (file_exists($new_file)) {
                            $thisFile = new Application_Service_Upload_PsendUploadFile;
                            $data['size'] = $thisFile->format_file_size($_FILES['ImageFile']['size']);

                            $request = $this->getRequest();
                            $data['category'] = ucwords($request->getParam('category'));
                            //   $data['userType'] = $request->getParam('userType');
                            $data['url'] = $newFileName . '.' . $file_ext;
                            $data['description'] = $request->getParam('caption');
                            $data['title'] = $files['ImageFile']['name'];
                            $data['uploader'] = $this->_auth->firstname . ' ' . $this->_auth->lastname;
                            $data['public_token'] = $this->_fm->generateToken(32);                            
                            $this->_fm->saveSalesFile($data);
                        } else	{
...
//	list($CurWidth,$CurHeight)=getimagesize($TempSrc);
	...
	//Resize image to Specified Size by calling resizeImage function.
       ...
       // $DestRandImageName = $dest_dir.'baby2.'.$file_ext;
        ...
        $thumb_DestRandImageName = $this->_downloadsDir.$ThumbPrefix. $data['url']; //Thumbnail name with destination directory
