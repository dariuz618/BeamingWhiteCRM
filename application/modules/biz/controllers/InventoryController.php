   ...
   /* public function ajaxUpdateShipmentAction() {
        ...
        if($_POST['action'] == 'add') {
           ...
           // var_dump($_POST);
            $data = array('item' => trim($itemParts[2]), 'qtyPerBox' => trim($_POST['qtyPerBox']), 
                ...
                'inventory_shipment_id'=> $_POST['inventory_shipment_id'], 'order_id'=>trim($_POST['orderid'])
                );
            $error = '';
            ...
            //get all quantities on hand regardless order number
            ...
            //find order on hand based on the order number             
            $order = $this->m_inventory->get_order($data['order_id']);                       
            if ($order['quantity_oh_china'] < $data['quantity']) {            
                echo "Order {$data['order_id']} only has {$order['quantity_oh_china']}  on stock, can not ship out {$data['quantity']} <br>";
                return;
            }
            //update order oh 4/15/2016
            ...
            $this->m_inventory->update_orders($orderUpdate, $data['order_id']); 
            
            /* update based on FIFO, automatically get order ids
            ...
            $transaction = array('order_id' => $data['order_id'],
    ...
    public function ajaxUpdateShipmentItemAction() {
       ...
       // $totalQtyOnHand = $this->m_inventory->get_oh_by_item($_POST['item']) + $_POST['originalQty'];
        $currentOrder = $this->m_inventory->get_order($_POST['orderid']);
        $totalQtyOnHand = $currentOrder['quantity_oh_china'] + $_POST['originalQty'];        
        if($totalQtyOnHand < $requestQty) {
            echo "Total On Hand for order {$_POST['orderid']}: $totalQtyOnHand,  $requestQty requested";
            return;
        }       
        ...
        $data = array("qtyPerBox" => $_POST['qtyPerBox'], "numberOfBoxes" => $_POST['numberOfBoxes'],
            ...
            'actualPrice' =>$_POST['actualPrice'], 'invoicePrice' => $_POST['invoicePrice'], 'order_id'=>trim($_POST['orderid']));
    ...
    public function commercialInvoiceAction() {
        ...
        if ($this->_getParam('export')) {
            ...
            header("Content-Disposition: attachment; filename=\"commercialInvoice_" . $shipmentId . ".xls\"");
        }
        ...
        if ($this->getRequest()->isPost()) {
            ...
            if (isset($_POST['createShipment'])) {
                ...
                for ($i = 0; $i < sizeof($_POST['item']); ++$i) {
                    if (trim($_POST['item'] != '' && trim($_POST['orderid'][$i]) && trim($_POST['quantity'][$i]))) {                  
                        $itemParts = preg_split('/[***]+/', trim($_POST['item'][$i]));                     
                        $rows[$itemParts[2]]['item'][] = $itemParts[2];    
                        $rows[$itemParts[2]]['order_id'][] = trim($_POST['orderid'][$i]);   
            ...
            if (empty($rows)) {
                $this->view->error = "Please Enter all required fields for items to be shipped.<br>";
            }
                     
           // echo '<pre>';
            foreach ($rows as $key=>$value) {              
               // var_dump($value['order_id'][0]);
                //$qoh = $this->m_inventory->get_oh_by_item($key);
                $qoh = $this->m_inventory->get_order($value['order_id'][0]);
                //var_dump($qoh);
                if ($qoh['quantity_oh_china'] < $value['quantity'][0]) {
                   $this->view->error .= "Order id {$value['order_id'][0]} for  Item $key only has {$qoh['quantity_oh_china']} OH, can not ship out {$value['quantity'][0]} <br>";
                }
            ...
            foreach ($rows as $key=>$row) {
                for($i = 0; $i < sizeof($rows[$key]['item']); ++$i) {                    
                   ...
                   // $order = $this->m_inventory->get_stock_by_item($shipmentItems['item']);        
                    $order = $this->m_inventory->get_order($shipmentItems['order_id']);
    ...
    public function createOrderAction() {        
		...
		$mail->setFrom('no-reply@beamingwhite.com', 'China Inventory System');
                $emailLists = array('adrian@beamingwhite.com', 'lucky@beamingwhite.com', 'loli@beamingwhite.com', 'luis@beamingwhite.com', 'pedidos@beamingwhite.com', 'tom@beamingwhite.com', 'mark@beamingwhite.com');
                foreach ($emailLists as $email){
                    $mail->addTo($email);
                }
                $mail->addBcc('jing@beamingwhite.com', 'webdev@beamingwhite.com');
    ...
    public function orderidAutosuggestAction() {   
        
        if($this->_getParam('term') != ''){
            $itemParts = preg_split('/[***]+/', $this->_getParam('term'));       
            $orders = $this->m_inventory->get_qoh_orders_by_item($itemParts[1]);          
            if($orders) {
