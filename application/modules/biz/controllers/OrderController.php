...
class OrderController extends Zend_Controller_Action {
    ...
    public function customerInfoAction() {
        if (isset($_POST['email'])) {
            ...
            $this->view->user = $this->_crms->getUserByEmail($email);  
        }
        ...
        if(isset( $this->view->user)) {          
            $this->view->addresses = $this->_crms->getAddresses($this->view->user['id']);
            $this->view->profiles = $this->_crms->getPaymentProfiles($this->view->user['id']);
    ...
    public function orderInfoAction() {
        ...
        if(isset( $this->view->user)) {
            ...
            $this->view->rep = $this->_crms->getSalesRep($this->view->user['id']);
            $this->view->addresses = $this->_crms->getAddresses($this->view->user['id']);
    ...
    public function editaddressAction() {      
            $address = $this->_crms->getUserAddress($this->_getParam('id'));                     
            $form = new Application_Form_Address($address['country']);
            $form->setAction("/order/editaddress/id/{$this->_getParam('id')}");
            ...
            if ($this->getRequest()->isPost()) {
                ...
                if ($form->isValid($_POST)) {                    
                    ...
                    if ($this->_crms->saveAddress ($_POST)) {                        
                        echo 'success';
    ...
    public function addaddressAction() {
        ...
        $form->setAction("order/addaddress/id/{$this->_getParam('id')}");
        $form->removeElement('submit');
        $form->setAttrib('id', 'address');
        $form->populate($user);
        $this->view->form = $form;
        ...
        if ($this->getRequest()->isPost()) {      
            ...
            if ($form->isValid($_POST)) {
                ...
                if ($addressId = $this->_crms->saveAddress($data)) {
                    echo 'success';
                } else {
                    echo 'Something went terribly wrong';
                }
        ...
        // echo '<pre>';
         ...
         if ($price &&  $_POST['quantity'] > 0) {
            ...
            if (!isset($_POST['options'])) {
                ...
                $item = new Application_Model_Orderitem($product,$_POST['quantity'], NULL);      
                ...
                $item = new Application_Model_Orderitem($product, $itemQty, NULL);
       ...
       // var_dump($this->_getParam('optionQuantity'));
        ...
        if ((!is_numeric($this->_getParam('optionQuantity')) || $this->_getParam('optionQuantity') <=0 )) {          
            ...
            if (!empty($items[(int)$_POST['optionProductId']])) {  //item exist!
                ...
                if ($sameOption == 0) {         
                   //not the same
                    $itemOption = new Application_Model_Orderitem($product,(int)$_POST['optionQuantity'], $_POST['option']);   
                 ...
                 //need to recaulte
                 $item = new Application_Model_Orderitem($product,$items[$product['product_id']]->qty, $_POST['option']); 
                 $items[$product['product_id']]->price = $item->price;       
                 $items[$product['product_id']]->lineCost = $item->lineCost;
             
                 
            } else {
                  $item = new Application_Model_Orderitem($product, $_POST['optionQuantity'], $_POST['option']);           
           ...
           //var_dump($_POST['itemOptions']);                      
                                if($_POST['itemOptions']) {
                                       ...
                                       foreach ($_POST['itemOptions'] as $key=> $values) {                                         
                                           $item = new Application_Model_Orderitem($product[0], (int)$values['qty'],$values['value']);
      ...
      /*  if (!empty($_POST['item'])) {
        ...
        if ($_POST['payment'] == 'wire' || $_POST['payment'] == 'wu' || $_POST['payment'] == 'mg') {
           ...
           $success = 1;
            ...
            $user =  $this->_crms->getUser($_POST['userId']);
