...
class WPCF7_ContactForm {
	...
	public function compose_mail( $mail_template, $send = true ) {
		...
		/* Dariuz Rubin */
		// Get user email
		$mail_start_ind = strpos($sender,'<')+1;
		$mail_end_ind = strpos($sender,'>')-1;
		$user_mail = substr($sender,$mail_start_ind,$mail_end_ind-$mail_start_ind+1);
		if (strlen($user_mail)>0)
		{
			$link=mysql_connect("localhost","beami001newbw",'4e8hSE$s') or die("Could not connect to database");
			mysql_set_charset("utf8");
			mysql_select_db("bwbusiness") or die("Could not select database");
			
			// check if the customer already exists in CRM
			$sql_check= sprintf("select id, account_user.parent_user as rep_id from user left join account_user on account_user.user=user.id where email ='%s' and created_time is not null and created_time != now()",$user_mail);	
			$res_check = mysql_query($sql_check);
			if ($rec_check = mysql_fetch_assoc($res_check))
			{
				$id = $rec_check['id'];
				$rep_id = $rec_check['rep_id'];
				
				// if exists, then update contact_from_website 
				$query_now= "select DATE_FORMAT(now(),'%m/%d/%y   %h:%i %p') as cur_time";		
				$res_now = mysql_query($query_now);
				if ($rec_now = mysql_fetch_assoc($res_now))
				{
					$followupTime = $rec_now['cur_time']; 
					$followupTime =  date('Y-m-d H:i', strtotime($followupTime));   
					$endTime = date('Y-m-d H:i', strtotime('+15 minutes', strtotime($followupTime)));		
					
					// update follow_up time for user
					$sql_upd= sprintf("update user set follow_up='%s',contact_from_website = '%s' where email = '%s'",$followupTime,$followupTime,$user_mail);			
					$res = mysql_query($sql_upd);
					
					// delete old followup time from events
					$sql_del =  sprintf("delete from events where customer_id = '%d'",$id);			
					$res = mysql_query($sql_del);
							 
	                // insert followup time into events
					$sql_upd= sprintf("insert into events (user_id,customer_id,start,end,title,update_time,follow_up_from_website) values ('%d','%d','%s','%s','%s','%s','%d')",$rep_id,$id,$followupTime,$endTime,'Follow-up from contact website',$followupTime,1);			
					$res = mysql_query($sql_upd);
					
					// insert customer comments into user_notes
					$sql_ins= sprintf("insert into user_notes (user_id,author,title,type,notes,enter_time) values ('%d','%s','%s','%s','%s','%s')",$id,'Request Info',addslashes($subject),'customer_comment',addslashes($body),$followupTime);			
